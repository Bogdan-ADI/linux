				     ...
				     const struct iio_chan_spec *chan, char *buf)
{
	...
	case BSYNC_DUTY_CYCLE:
		...
		rem = do_div(tdc_result, 1000000U);
			   ...
			   u32 vco_out_freq_hz)
{
	...
	ret = regmap_read_poll_timeout(st->regmap, ADF4030_REG(0x90),
				       ...
				       2000, 500000);
	if (ret) {
		dev_err(&st->spi->dev, "PLL failed to lock\n");
		return ret;
	}
...
static int adf4030_parse_fw(struct adf4030_state *st)
{
	...
	ret = device_property_read_string_array(dev, "clock-output-names",
						st->clk_out_names,
	...
	device_for_each_child_node_scoped(dev, child) {
		...
		/* Is this really needed?! If so we need to make it work though IIO labels */
...
void adf4030_stop_jesd204(void *jdev)
{
